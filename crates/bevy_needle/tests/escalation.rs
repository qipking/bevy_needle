...
fn mock_driver_failure_with_no_more_tiers_is_failed() {
    ...
    );
    {
        ...
        registry.register_for_tier(
            Arc::new(
                MockDriver::new("mock").with_script(vec![MockStep::fail(
    ...
    assert!(failed[0].contains("driver"), "failure should mention driver: {}", failed[0]);
}
...
fn no_driver_is_escalated_not_failed() {
    ...
    assert_eq!(escalated, 1, "no driver → Escalated (normal close), not Failed");
}
