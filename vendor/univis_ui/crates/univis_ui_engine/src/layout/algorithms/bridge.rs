...
pub fn final_size_with_indices(
    ...
    let temp_data: Vec<_> = normal_indices
        .iter()
        .map(|&i| {
            let item = &items[i];
    ...
    let mut temp_results: Vec<SolverResult> = temp_data.iter().map(|(_, _, res, _)| *res).collect();
    ...
    let placer: Box<dyn LayoutPlacer> = match layout.display {
        ...
        UDisplay::Grid => Box::new(GridPlacer {
            columns: layout.grid_columns as usize,
        }),
        UDisplay::Masonry => Box::new(MasonryPlacer {
            columns: layout.grid_columns as usize,
        }),
...
pub fn calculate_cross_align(child_cross_size: f32, ctx: &PlacementContext) -> f32 {
    ...
    match ctx.align_items {
        UAlignItems::Start | UAlignItems::Stretch | UAlignItems::FlexStart => {
